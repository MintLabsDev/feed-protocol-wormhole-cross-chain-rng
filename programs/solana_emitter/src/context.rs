...
use crate::{
    error::SolanaEmitterError,
    message::SolanaEmitterMessage,
...
pub struct RegisterEmitter<'info> {
    ...
    #[account(
        has_one = owner @ SolanaEmitterError::OwnerOnly,
        seeds = [Config::SEED_PREFIX],
        bump
    )]
...
type SolanaEmitterVaa = wormhole::PostedVaa<SolanaEmitterMessage>;

#[derive(Accounts)]
#[instruction(vaa_hash: [u8; 32])]
pub struct ReceiveRequestAndPostMessage<'info> {
    #[account(mut)]
    ...
    pub posted: Account<'info, SolanaEmitterVaa>,
    ...
    #[account(
        ...
        bump,
        constraint = foreign_emitter.verify(posted.emitter_address()) @ SolanaEmitterError::InvalidForeignEmitter
    )]
    ...
    #[account(
        mut,
        address = config.wormhole.bridge @ SolanaEmitterError::InvalidWormholeConfig
    )]
    ...
    #[account(
        mut,
        address = config.wormhole.fee_collector @ SolanaEmitterError::InvalidWormholeFeeCollector
    )]
    ...
    #[account(
        mut,
        address = config.wormhole.sequence @ SolanaEmitterError::InvalidWormholeSequence
    )]
