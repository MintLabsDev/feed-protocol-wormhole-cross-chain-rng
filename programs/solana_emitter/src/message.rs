...
pub enum SolanaEmitterMessage {
    Alive { program_id: Pubkey },
    Hello { message: Vec<u8> },
}

impl AnchorSerialize for SolanaEmitterMessage {
    fn serialize<W: io::Write>(&self, writer: &mut W) -> io::Result<()> {
        match self {
            SolanaEmitterMessage::Alive { program_id } => {
                PAYLOAD_ID_ALIVE.serialize(writer)?;
                program_id.serialize(writer)
            }
            SolanaEmitterMessage::Hello { message } => {
...
impl AnchorDeserialize for SolanaEmitterMessage {
    fn deserialize_reader<R: io::Read>(reader: &mut R) -> io::Result<Self> {
        match u8::read(reader)? {
            PAYLOAD_ID_ALIVE => Ok(SolanaEmitterMessage::Alive {
            ...
            PAYLOAD_ID_HELLO => {
                ...
                if length > HELLO_MESSAGE_MAX_LENGTH {
                    ...
                    Ok(SolanaEmitterMessage::Hello { message: buf })
                }
            }
            _ => Err(io::Error::new(
                io::ErrorKind::InvalidInput,
...
mod test {
    ...
    fn test_message_alive() -> Result<()> {
        ...
        let msg = SolanaEmitterMessage::Alive {
        ...
        match SolanaEmitterMessage::deserialize(&mut encoded.as_slice())? {
            SolanaEmitterMessage::Alive { program_id } => {
    ...
    fn test_message_hello() -> Result<()> {
        ...
        let msg = SolanaEmitterMessage::Hello {
